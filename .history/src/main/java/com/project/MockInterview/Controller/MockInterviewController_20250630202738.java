...
public class MockInterviewController {
    ...
    public ResponseEntity<Map<String, List<String>>> generateQuestion(@RequestBody QuestionRequest req) {
        String prompt = String.format(
                "Act as a technical interviewer. Generate 3 entry-level mock interview questions for a candidate applying for the role of '%s' with skills: %s. "
                        +
                        "Return only the questions in a clean JSON array format, like this: " +
    ...
    private List<String> extractQuestions(String response) {
        ...
        Pattern pattern = Pattern.compile("\\d+\\.\\s+(.*?)\\n"); // Matches lines like: 1. What is Java?
        ...
        // Fallback: If pattern fails, just split by newlines and use all reasonable
        // lines
